...
from Lovechat.database import Chiku
...
async def cb_handler(_, query: CallbackQuery):
    if query.data == "HELP":
        ...
        await query.answer("𝙲𝙻𝙾𝚂𝙴ᴅ ᴍᴇɴᴜ!", show_alert=True)
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_Chiku = Chiku.find_one({"chat_id": query.message.chat.id})
            if not is_Chiku:
                await query.edit_message_text(f"**ᴄʜᴀᴛ-ʙᴏᴛ ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ.**")
            if is_Chiku:
                Chiku.delete_one({"chat_id": query.message.chat.id})
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_Chiku = Chiku.find_one({"chat_id": query.message.chat.id})
            if not is_Chiku:
                Chiku.insert_one({"chat_id": query.message.chat.id})
                await query.edit_message_text(
